...
class Gesture:
    def __init__(self, sim, target_handle, speed=0.01):
        self.sim = sim
    ...
    def move(self, dx=0, dy=0, dz=0):
        ...
        target_pos[1] += dy
        target_pos[2] += dz
        self.sim.setObjectPosition(self.target_handle, -1, target_pos)

    def gesture_control(self, hand_sign_text):
        if hand_sign_text == "Forward":
            ...
            self.move(dz=self.speed)
        elif hand_sign_text == "Down":
            self.move(dz=-self.speed)
        elif hand_sign_text == "Take Off":
            self.sim.setObjectPosition(self.target_handle, -1, [0, 0, 1])
        elif hand_sign_text == "Land":
            self.sim.setObjectPosition(self.target_handle, -1, [0, 0, 0])
...
def main():
    ...
    gesture = Gesture(sim, target_handle)
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # Prints the values of the landmarks
                ...
                # Prints the values of the landmarks in a list
                # print(landmark_list[0])  # displays pixel coordinates
                ...
                gesture.gesture_control(hand_sign_text)
...
def pre_process_landmark(landmark_list):
    ...
    # Normalization
    # Gets the absolute distance (removes negatives)
    # Example: [-1, -4, -6] -> [1, 4, 6]
...
def log_csv(number, mode, landmark_list, point_history_list):
    if mode == 0:
        pass
...
def create_landmarks(image, landmark_point):
...
def draw_point_history(image, point_history):
    for index, point in enumerate(point_history):
        if point[0] != 0 and point[1] != 0:
            cv.circle(image, (point[0], point[1]), 1 + int(index / 2), (0, 0, 0), 2)
    return image
...
def draw_info(image, fps, mode, number):
    ...
    mode_string = ['Logging Key Points', 'Logging Point History']
